import { INTERACTIVE_BASE_CLASSES } from '../interactiveBase'
...
const sizeClasses: Record<CloseButtonSize, string> = {
  [CloseButtonSize.Sm]: [
    'size-5 transition-[color,background-color] duration-[120ms] active:bg-surface-pressed-strong',
...
export function CloseButton({ size = CloseButtonSize.Md, className = '', ...rest }: CloseButtonProps) {
  return (
    <button
      type="button"
      className={[
        'relative flex flex-none items-center justify-center rounded-sm text-content-secondary',
        'hover:bg-surface-pressed hover:text-content-primary',
        'focus-ring',
