 ...
 *
 * Tab is caught only when it would actually complete something (a match for the current prefix); with no
 * match, or with Shift held, Tab moves focus like it does everywhere else — the terminal never traps it.
 */
  ...
  const inputProps = {
    ...
    onKeyDown(e: KeyboardEvent<HTMLInputElement>) {
      if (e.key === 'Escape') {
        ...
        const prefix = value.trim().toLowerCase()
        const matches = completion.current?.matches ?? completions.filter((c) => c.startsWith(prefix))
        if (matches.length === 0) return // nothing to complete — let Tab move focus forward as usual
        e.preventDefault()
        complete()
      }
    },
