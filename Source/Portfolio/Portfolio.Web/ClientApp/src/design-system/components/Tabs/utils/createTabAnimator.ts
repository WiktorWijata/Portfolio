...
export function createTabAnimator(container: HTMLElement, reducedMotion: MediaQueryList) {
  ...
  /** Cancels every animation still running (e.g. neighbours sliding into place) — for a cancelled gesture or unmount. */
  function cancelAll() {
  ...
  return { cancel, cancelAll, layoutLeft, animateOrder, animateDrop }
}
