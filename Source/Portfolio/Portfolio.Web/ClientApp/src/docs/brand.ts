import { readThemeColor } from '@/design-system'
...
export function markFaviconHref() {
  // A data URI cannot use CSS variables, so it takes the current values of the theme tokens.
  const background = readThemeColor('surface-panel')
  const petalColor = readThemeColor('accent')
  ...
  const svg =
    ...
    `<g transform="translate(16 16) scale(.8) translate(-16 -15)" fill="${petalColor}">${petals}` +
    `<path d="${MARK_LIP}"/><circle cx="${MARK_COLUMN.cx}" cy="${MARK_COLUMN.cy}" r="${MARK_COLUMN.r}" fill="${background}" opacity=".85"/></g></svg>`
