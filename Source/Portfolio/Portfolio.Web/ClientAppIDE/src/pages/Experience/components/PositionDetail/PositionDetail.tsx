...
export function PositionDetail({ position }: PositionDetailProps) {
  return (
    <article>
      <header className="border-b border-b-border pb-4">
        <div className="font-mono text-[13px] leading-[normal]">
          <Text size={FontSize.XSmall} className="leading-[normal] tracking-[.05em] text-text-label">
            {position.period}
          </Text>
        </div>
        <Text
          ...
          className="mt-1.5 mb-0.5 text-[22px] leading-[normal] tracking-[-.01em] text-text"
        >
          {position.role}
        </Text>
      ...
      {position.groups.map((group) => (
        <section
          key={group.title}
          className="grid grid-cols-[130px_minmax(0,1fr)] gap-4 border-b border-b-bar-subtle py-3.5 max-[700px]:grid-cols-1 max-[700px]:gap-1.5 @max-[560px]:grid-cols-1 @max-[560px]:gap-1.5"
        >
          <Text
            as="h4"
            ...
            weight={FontWeight.SemiBold}
            className="mt-0.5 leading-[normal] tracking-[.1em] text-text-faint uppercase"
          >
            {group.title}
          </Text>
          <ul className="m-0 flex list-none flex-col gap-[7px] p-0">
            {group.items.map((item) => (
              <li key={item} className="relative pl-4">
                <span aria-hidden className="absolute left-0 text-text-faint">
                  –
                </span>
                <Text
                  as="p"
                  font={FontFamily.Sans}
                  className="text-[13.5px] leading-[1.6] [text-wrap:pretty] text-[#b0b3b8]"
                >
                  {item}
                </Text>
              </li>
