...
const cardClasses = 'rounded-xl border border-border-2 bg-list/80 px-4 py-3.5'
const arrowButton =
  'grid h-7 w-6 cursor-pointer place-items-center rounded-sm border-0 bg-transparent p-0 text-[#aaa2ae] transition-colors duration-150 ease-out hover:bg-accent/[.082] hover:text-[#e3badb] outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent'
  ...
  return (
    <div className={`${cardClasses} relative pr-[100px]`}>
      ...
      <Text
        ...
        aria-live="polite"
        aria-atomic="true"
        className="mt-[3px] block min-h-[2.8em] leading-[1.4] text-text-dim"
      >
      ...
      >
        ...
        <Text aria-hidden font={FontFamily.Mono} className="text-[10px] whitespace-nowrap text-[#83818d]">
    ...
    <div className="mt-[30px] grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-3">
      {HOME_METRICS.map((metric) => (
        <div key={metric.label} className={cardClasses}>
          ...
          <Text as="span" size={FontSize.Small} font={FontFamily.Sans} className="mt-[3px] block text-text-dim">
            {metric.label}
          </Text>
        </div>
      ))}
