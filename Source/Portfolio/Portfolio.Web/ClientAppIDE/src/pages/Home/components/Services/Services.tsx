...
export function Services() {
  return (
    <section
      ...
      className="mt-auto mb-auto translate-y-[15px] border-t border-border pt-5 max-[1000px]:mt-[30px] max-[1000px]:mb-0 max-[1000px]:translate-y-0"
    >
      <Text
        as="h2"
        ...
        className="mb-[18px] leading-tight tracking-[-.3px] text-text-heading"
      >
        {HOME_SERVICES_TITLE}
      </Text>
          ...
          <div key={service.title} className="grid grid-cols-[18px_minmax(0,1fr)] items-start gap-[9px]">
            ...
            <div>
              <Text
                ...
                font={FontFamily.Sans}
                weight={FontWeight.Medium}
                className="mb-1.5 leading-[1.4] text-text-row-title"
              >
                {service.title}
              </Text>
              <Text as="p" size={FontSize.Small} font={FontFamily.Sans} className="leading-[1.65] text-[#969ba5]">
                {service.text}
              </Text>
            </div>
          </div>
