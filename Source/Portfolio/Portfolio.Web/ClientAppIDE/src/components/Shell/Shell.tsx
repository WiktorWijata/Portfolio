...
export function Shell() {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border border-border bg-editor">
      <AppTitleBar />
      <div className={`grid min-h-0 flex-1 ${explorerOpen ? GRID_EXPLORER_OPEN : GRID_EXPLORER_COLLAPSED}`}>
        <Rail />
        {explorerOpen && (
          <div className="flex min-h-0 max-bp570:max-h-[200px] max-bp570:border-b max-bp570:border-border-4">
            <Explorer />
          </div>
        )}
        <EditorArea />
