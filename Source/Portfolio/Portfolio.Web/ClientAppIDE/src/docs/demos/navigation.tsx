    ...
    <div className="flex w-full flex-col gap-2">
      <Tabs aria-label="Otwarte zakładki" className="overflow-hidden rounded-t-md" onReorder={setOpenTabs}>
        ...
        {!openTabs.includes('home') && (
          <button
            ...
            className="self-center px-3 font-mono text-xs text-text-dim hover:text-text"
          >
            + otwórz „O mnie"
          </button>
        )}
      ...
      <div className="bg-editor p-4">
        <Text size={FontSize.Small} color={TextColor.Body}>
          Zawartość zakładki: <strong className="text-text">{tabLabels[activeTab] ?? '(brak otwartych)'}</strong>
        </Text>
      </div>
    </div>
  )
...
const explorerTree: ExplorerNode[] = [
  { type: 'file', id: 'getstarted', label: 'GetStarted.md', icon: <FileText className="size-4 text-[#8eb5df]" /> },
  {
    type: 'folder',
    id: 'sln',
    ...
    children: [
      {
        ...
        children: [{ type: 'file', id: 'portfolio', label: 'Portfolio.cs', icon: csIcon('text-[#6fb882]') }],
      },
      { type: 'file', id: 'stack', label: 'Stack.cs', icon: csIcon('text-[#8fd18f]') },
      { type: 'file', id: 'experience', label: 'Experience.cs', icon: csIcon('text-[#6fb882]') },
      { type: 'file', id: 'contact', label: 'Contact.cs', icon: csIcon('text-[#6fb882]') },
    ],
  },
]
...
>
  <TreeFolder label="WiktorWijata.sln" icon={<Boxes />}>
    ...
    preview: (
      <div className="w-[190px] overflow-hidden border border-border">
        <SolutionExplorer className="w-full border-r-0" aria-label="Podgląd explorera">
          <TreeFile icon={csIcon('text-[#6fb882]')}>Portfolio.cs</TreeFile>
          <TreeFile active icon={csIcon('text-[#8fd18f]')}>
            Stack.cs
          </TreeFile>
        </SolutionExplorer>
      </div>
