  ...
  return (
    <div className="flex w-full flex-col gap-3">
      {Object.entries(ContainerSize).map(([name, size]) => (
        <div key={size} className="rounded-sm border border-dashed border-border-5 bg-editor py-3">
          <Container size={size}>
...
function SplitPanelDemo() {
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      ...
      >
        <div className="h-full bg-editor p-5">
          <Text as="p" size={FontSize.Large} color={TextColor.Body}>
...
<Container size={ContainerSize.Wide}>Układ z sidebarem (max 1280 px)</Container>`,
    ...
    preview: (
      <div className="flex h-14 w-full items-center justify-center rounded-sm border border-dashed border-border-5">
        <div className="h-8 w-2/3 rounded-sm bg-hover" />
      </div>
    ),
    Demo: ContainerDemo,
  },
...
>
  <div className="p-5">Szczegóły wybranego stanowiska</div>
    ...
    preview: (
      <div className="grid h-14 w-full grid-cols-[1fr_2fr] overflow-hidden rounded-md border border-border">
        <div className="border-r border-border bg-list" />
...
<StatusBar aria-label="Pasek statusu">
  ...
  <StatusBarItem tone={StatusBarTone.Faint}>v0.1.0</StatusBarItem>
    ...
    preview: (
      <div className="flex h-7 w-full items-center gap-3 border-t border-border bg-hover px-3">
        <span className="font-mono text-xs text-status-branch">⑂ master</span>
        <span className="ml-auto rounded-xs bg-lang-active px-[5px] py-[3px] font-mono text-2xs text-lang-active-text">
          PL
        </span>
      </div>
    ),
...
<TitleBar logo="W_" title="WiktorWijata / Portfolio">
  ...
  <TitleBarButton tone={TitleBarButtonTone.Link} onClick={toggleExplorer}>☰ Explorer</TitleBarButton>
    ...
    preview: (
      <div className="flex h-9 w-full items-center gap-3.5 border-b border-border bg-hover px-3">
        <span className="rounded-[2px] bg-accent px-[5px] py-[3px] font-mono text-xs font-semibold text-on-accent">
          W_
        </span>
        <span className="rounded-md border border-run-border bg-run-bg px-2.5 py-1 font-sans text-xs font-medium text-run-text">
          ▷ Oprowadź mnie
        </span>
      </div>
    ),
