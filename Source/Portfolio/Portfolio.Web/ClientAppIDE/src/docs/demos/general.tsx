...
import { Terminal as TerminalIcon, MessageSquare, Search, User, FolderKanban, Route, Layers, Mail } from 'lucide-react'
import {
  Button,
  ButtonVariant,
  FontFamily,
...
function IconButtonDemo() {
  return (
    <>
      <IconButton icon={<TerminalIcon />} aria-label="Terminal" />
      <IconButton icon={<MessageSquare />} aria-label="Asystent" active />
      <IconButton icon={<Search />} aria-label="Szukaj" />
    </>
  )
}
...
<Button type="submit" size={ButtonSize.Lg}>Wyślij wiadomość ↗</Button>`,
  ...
  },
  {
    ...
    usage: `import { IconButton, IconButtonSize } from './design-system'

<IconButton icon={<Terminal />} aria-label="Terminal" active={terminalOpen} onClick={toggleTerminal} />
<IconButton size={IconButtonSize.Sm} icon={<ListTree />} aria-label="Rozwiń wszystkie foldery" />`,
    name: 'IconButton',
    category: 'general',
    summary: 'Przycisk z samą ikoną w dwóch rozmiarach (Md i kompaktowy Sm).',
    preview: (
