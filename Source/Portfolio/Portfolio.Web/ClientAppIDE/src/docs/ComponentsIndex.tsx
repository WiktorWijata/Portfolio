...
function ComponentCard({ entry }: { entry: DocEntry }) {
  return (
    <a
      ...
      className="block rounded-3xl outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
    >
      <Panel interactive className="flex h-full flex-col overflow-hidden">
        <div
          inert
          className="pointer-events-none flex h-[132px] items-center justify-center overflow-hidden border-b border-border-5 bg-editor p-4"
        >
          {entry.preview}
        </div>
      ...
      {categories.map((category) => {
        ...
        return (
          <section key={category.id} className="flex flex-col gap-4">
            <div className="flex items-center gap-3 border-b border-border-2 pb-3">
